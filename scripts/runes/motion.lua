...
storage.thaumfactory.active_motion_modifiers = {}
  ...
  function()
    for _, player in pairs(game.players) do
      if player.controller_type == defines.controllers.character then
        ...
        if rune then
          if not storage.thaumfactory.active_motion_modifiers[player.index] then
            -- case 1: player has inactive rune - track item number and modify speed
            if rune.durability >= 1 then
              storage.thaumfactory.active_motion_modifiers[player.index] = { item_number = rune.item_number, distance = 0 }
              player.character_running_speed_modifier = 0.3
            end
          else
            -- case 2: player has active rune but no durability - reset
            if rune.durability <= 1 then
              storage.thaumfactory.active_motion_modifiers[player.index] = nil
...
script.on_event(defines.events.on_player_changed_position,
  function(event)
    ...
    if player.controller_type == defines.controllers.character then
      local rune = player.get_main_inventory().find_item_stack("thaumfactory-motion-rune-stone")
      local motion = storage.thaumfactory.active_motion_modifiers[player.index]
